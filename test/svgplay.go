...
// handleFrontPage is an HTTP handler that renders the svgplay interface.
...
// handleCompile is an HTTP handler that reads Go source code from the request,
...
<html>
<head>
<title>svgplay: gensvg sketching</title>
<style>
:root {
    --main-color: #3c3c3c;
...
window.addEventListener('DOMContentLoaded',function () {
	if(document.getElementById("intercompile").checked) {
		compile();
		setInterval(compile, 5000);
	}
});
...
<div class="hints">
(Shift-Enter to compile and run.)<br />
<input type="checkbox" id="autocompile" value="checked" /> Compile and run after each keystroke<br />
<input type="checkbox" id="intercompile" value="checked" /> Compile every 5s
</div>
<td width="3%">
