...
func WithGradeHeight(gradeHeight float64) RenderOptions {
	return func(options *renderOptions) {
		options.gradeHeight = gradeHeight
	}
}
...
// gradeTallyNodeProperties holds temporarily some properties that would be wasteful to re-compute
...
func RenderLinearProfileSVG(
	...
	if cfg.height == -1.0 {
		cfg.height = amountOfProposals*cfg.gradeHeight + totalVerticalSpacing + doublePadding
	}
	...
	height := cfg.height - doublePadding
	width := cfg.width - doublePadding
	...
	for proposalIndex, proposal := range proposals {
		...
		lineX := width * 0.5
		if int(amountOfJudges)&1 == 0 {
			lineXOffset := 0.0
			lineXPotentialOffset := 0.5 * width / amountOfJudges
			if cfg.medianLineStrategy == FavorAdhesion {
				lineXOffset = lineXPotentialOffset
			}
			if cfg.medianLineStrategy == FavorContestation {
				lineXOffset = -lineXPotentialOffset
			}
			if cfg.bestGradeOnLeft {
